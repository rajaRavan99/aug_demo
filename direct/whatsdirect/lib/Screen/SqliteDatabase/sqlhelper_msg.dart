...
class SqlDatabaseHelper {
    ...
    return openDatabase(
      ...
      onCreate: (database, version) async {
        await database.execute(
          "CREATE TABLE msg(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, subtitle TEXT,isSelected INTEGER )",
        );

        await database.execute(
          "CREATE TABLE $table2(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, subtitle TEXT)",
        );
      },
      version: 1,
    );
  ...
  Future<List<MsgData>> queryAllRow() async {
    ...
    print("----- Data Get -----> $msgList");
    return msgList;
  }

  Future<List<NoteModel>> getAllRecords() async {
    final db = await instance.database;
    var result = await db.rawQuery("SELECT * FROM $table2");
    var newList = result.map((e) => NoteModel.fromMap(e)).toList();
  ...
  Future<int> customInsertNote(List<NoteModel> noteModel) async {
    int res = 0;
    Database db = await instance.database;
    for (var msg in noteModel) {
      res = await db.insert(table2, msg.toMap());
    }
    return res;
  }
  ...
  Future<int> insertNote(NoteModel noteModel) async {
    Database db = await instance.database;
    var res = await db.insert(table2, noteModel.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
    print('---------insertNote--------> ${res}');
    return res;
  }
  ...
  Future<int> updateNote(NoteModel noteModel) async {
    Database db = await instance.database;
    var res = await db.update(table2, noteModel.toMap(),
